...
prettyTable <- function(x, num_cols=NULL) {
    ...
    ## Do num cols first
    d_num <- ldply(x[,num_cols,drop=F], function(x) c(round(median(x,na.rm=T),1),min(x,na.rm=T),max(x,na.rm=T),sum(is.na(x))))
    ...
    # Transform to counts
    d <- ldply(x[,i,drop=F], function(x) t(rbind(names(table(x,useNA="ifany")),table(x,useNA="ifany"),paste0(round(prop.table(table(x,useNA="ifany"))*100,1),"%"))))
